 ...
 */
template <typename Dtype>
class SequenceInputLayer : public BasePrefetchingDataLayer<Dtype> {
 public:
  explicit SequenceInputLayer(const LayerParameter& param)
      : BasePrefetchingDataLayer<Dtype>(param) {}
  virtual ~SequenceInputLayer();
  virtual void DataLayerSetUp(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top);

  virtual inline const char* type() const { return "SequenceInput"; }
  virtual inline int ExactNumBottomBlobs() const { return 0; }
  // bottom 0 [data]
  // bottom 1 [label and clip_masks], size is frames * buffer*(num_label + 1)
  virtual inline int ExactNumTopBlobs() const { return 2; }
